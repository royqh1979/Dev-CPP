...
type
  ...
  public
    ...
    procedure OpenFile(const FileName: String;OpenUseUTF8:boolean);
    ...
    procedure AddFindOutputItem(const line, col, filename, msg:String;wordlen:integer);
...
procedure TMainForm.OpenFile(const FileName: String; OpenUseUTF8: boolean);
var
  e: TEditor;
begin
...
procedure TMainForm.AddFindOutputItem(const line, col, filename, msg: String; wordlen:integer);
var
  ListItem: TListItem;
begin
...
function CompareFileModifyTime(FileName1:String; FileName2:String):TValueRelationShip;
var
...
begin
  ...
  if idx > -1 then begin // devcpp-based
    ...
    ShellExecute(0, 'open',
      pChar(devDirs.Exec + 'ResEd/ResEd.exe'),
      pChar(fProject.Units[idx2].FileName),
      pChar(ExtractFilePath(fProject.Units[idx2].FileName)),
      SW_SHOW);
  end
...
begin
  ...
  if Assigned(Editor) then begin
    ...
    // MessageBox(Application.Handle,pChar(Concat(word,IntToStr(offset))),     PChar( 'Look'), MB_OK);
    if Word = '' then begin
      Exit;
    end;
...
begin
  ...
  if Assigned(e) then begin
    ...
    if Folder <> '' then begin
      ...
      ShellExecute(Application.Handle, 'open', 'cmd',  nil,pChar(Folder), SW_SHOWNORMAL);
    end;
  end;
end;
